...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('loadFiles').onclick = async () => {
        ...
        try {
            ...
            info.innerHTML = `✅ Файлы загружены!<br>Файл 1: ${(loadedSamples1.length/loadedRate1).toFixed(1)}с, ${loadedRate1} Гц<br>Файл 2: ${(loadedSamples2.length/loadedRate2).toFixed(1)}с, ${loadedRate2} Гц`;
        } catch (err) {
    ...
    document.getElementById('applyMix').onclick = () => {
        ...
        plotSignal(currentSamples, 'signalPlot');
        plotSpectrum(currentSamples, 'spectrumPlot');
        info.innerHTML = `🎛️ Применено микширование: баланс ${mix1.value} : ${mix2.value}`;
    };
    ...
    document.getElementById('playSound').onclick = () => {
        if (currentSamples) playSignal(currentSamples, currentRate);
        else info.innerHTML = '⚠️ Нет сигнала для воспроизведения';
    };
